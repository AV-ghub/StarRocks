...
А теперь, при попытке загрузить данные по [инструкции](https://docs.starrocks.io/docs/quick_start/shared-data/#new-york-city-collision-data---crashes) мы по факту загружаем их в [соседний контейнер](https://docs.starrocks.io/docs/quick_start/shared-nothing/#launch-starrocks)

Объяснение этому и план действий следующий

Отличный вопрос! Вы столкнулись с тонким, но важным отличием в процессе загрузки данных между архитектурами **Shared-nothing** и **Shared-data**. Проблема в том, что порт `8030`, который сработал, принадлежит контейнеру `starrocks-fe`, и данные загрузились именно в него, в то время как в архитектуре с разделением хранилища и вычислений загрузкой должен управлять Compute Node (CN).

### 🔍 Сравнение загрузки данных в разных архитектурах

Вот ключевые отличия, которые помогут понять причину проблемы:

| Аспект | **Shared-Nothing** Архитектура | **Shared-Data** Архитектура |
| :--- | :--- | :--- |
| **Целевой узел для загрузки** | Backend (BE) | Compute Node (CN) |
| **Порт HTTP для загрузки** | Порт **BE** (обычно `8040`)  | Порт **CN** (у вас `8540`) |
| **Роль узла** | BE хранит данные и выполняет вычисления  | CN — безсостояственный узел, выполняет вычисления  |
| **В вашем случае** | Команда из руководства использует `8030` (FE) | Нужно использовать `8540` (CN) |

### 🛠️ Как исправить загрузку для Shared-Data

Вам нужно модифицировать команду `curl`, указав в ней правильный хост и порт, где слушает ваш Compute Node.

1.  **Проверьте статус CN**: Убедитесь, что контейнер `starrocks-cn` здоров и ему проброшен порт `8540`.
    ```bash
    sudo docker compose ps
    ```

2.  **Измените команду загрузки**: Исправьте URL в команде `curl`. Вместо `http://localhost:8030/...` используйте `http://localhost:8540/...`.
    ```bash
...
В архитектуре **Shared-data** данные хранятся не на вычислительных узлах, а в удаленном объектном хранилище (в вашем случае MinIO). Процесс загрузки выглядит так :
1.  Запрос на загрузку (`curl`) поступает на Compute Node (CN).
2.  CN обрабатывает данные и записывает их непосредственно в объектное хранилище (MinIO) через настроенный `STORAGE VOLUME`.
3.  FE координирует процесс и обновляет метаданные.

Таким образом, порт `8030` (FE) не предназначен для приема больших объемов данных в этой архитектуре, его роль — управление метаданными и планирование запросов.

Попробуйте выполнить загрузку через порт `8540`, и процесс должен завершиться успешно. Если снова возникнут ошибки, проверьте логи CN с помощью `sudo docker compose logs starrocks-cn` — они помогут точнее диагностировать проблему.
